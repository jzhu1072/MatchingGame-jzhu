...
function shuffleCardImageClasses() {
  ...
  cardClassesShuffled = _.shuffle(cardClasses);
...
function onCardFlipped(newlyFlippedCard) {
  /* Step 1: Use the 'incrementCounter' function to add one to the flip counter UI.  */
	incrementCounter(counters, document.getElementById("flip-count"));
  ...
  if (counters["matches"] === 6 && doCardsMatch(lastCardFlipped, newlyFlippedCard) === "True") {
    winAudio.play();
  } else if (doCardsMatch(lastCardFlipped, newlyFlippedCard) === "True") {
    matchAudio.play();
...
function resetGame() {
  // Pause music when reset button clicked
  winAudio.pause();
